        ...
        function testProgressModal() {
            console.log('🧪 测试进度弹窗显示');
            // 直接调用showProgressModal，确保正确的显示逻辑
            showProgressModal();
        }
        ...
        // 调试函数：查看退出历史记录
        function debugExitHistory() {
            const exitCount = parseInt(localStorage.getItem('exitCount') || '0');
            const exitHistory = JSON.parse(localStorage.getItem('exitHistory') || '[]');
            const readingTime = parseInt(localStorage.getItem('readingTime') || '0');
            const pageEnterTime = localStorage.getItem('pageEnterTime');
            
            console.log('🔍 退出调试信息:');
            console.log('总退出次数:', exitCount);
            console.log('当前阅读时间:', readingTime, '秒');
            console.log('页面进入时间:', pageEnterTime ? new Date(parseInt(pageEnterTime)).toLocaleString() : '未记录');
            console.log('退出历史记录数量:', exitHistory.length);
            console.log('退出历史记录:', exitHistory);
            
            if (exitHistory.length > 0) {
                console.log('最近一次退出:', exitHistory[exitHistory.length - 1]);
                
                // 计算总退出时长
                const totalExitDuration = exitHistory.reduce((sum, record) => sum + record.duration, 0);
                console.log('总退出时长:', totalExitDuration, '秒');
                
                // 计算实际阅读时间（总时间 - 退出时间）
                const actualReadingTime = readingTime - totalExitDuration;
                console.log('实际阅读时间（排除退出时间）:', actualReadingTime, '秒');
            }
        }
        
        // 重置退出数据
        function resetExitData() {
            if (confirm('确定要重置退出数据吗？这将清空所有退出记录。')) {
                console.log('🔄 开始重置退出数据...');
                
                // 清空退出相关数据
                localStorage.removeItem('exitCount');
                localStorage.removeItem('exitHistory');
        ...
        // 测试退出跟踪状态
        function testExitTrackingState() {
            const state = window.exitTrackingState;
            const exitCount = parseInt(localStorage.getItem('exitCount') || '0');
            const exitHistory = JSON.parse(localStorage.getItem('exitHistory') || '[]');
            
            console.log('🧪 退出跟踪状态测试:');
            console.log('全局状态:', state);
            console.log('localStorage退出次数:', exitCount);
            console.log('localStorage退出历史:', exitHistory);
            console.log('localStorage退出历史数量:', exitHistory.length);
            
            if (state) {
                console.log('✅ 全局状态存在');
                console.log('页面可见性:', state.isPageVisible);
                console.log('是否在跟踪退出:', state.isTrackingExit);
                console.log('是否初始加载:', state.isInitialLoad);
                console.log('退出开始时间:', state.exitStartTime ? new Date(state.exitStartTime).toLocaleTimeString() : 'null');
                console.log('退出次数:', state.exitCount);
            } else {
                console.log('❌ 全局状态不存在');
            }
        }
        ...
        // 手动记录退出
        function manualRecordExit() {
            const exitCount = parseInt(localStorage.getItem('exitCount') || '0') + 1;
            ...
            const exitRecord = {
                ...
                duration: 5, // 模拟5秒退出
                exitNumber: exitCount,
                exitStartTime: new Date(Date.now() - 5000).toISOString(),
                exitEndTime: new Date().toISOString(),
                reason: 'manual_test'
            };
            ...
            console.log(`📱 手动记录退出，总退出次数: ${exitCount}`);
            console.log('📱 手动退出记录详情:', exitRecord);
            
            // 更新recording弹窗中的退出次数显示
            updateRecordingExitCount();
            
            alert(`手动记录退出成功！当前退出次数: ${exitCount}`);
        }
